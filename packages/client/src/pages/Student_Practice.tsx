...
export default function StudentPractice() {
  ...
  const lastPredictionRef = useRef<{ label: string; time: number } | null>(null);
  ...
  const captureAndPredictFrame = async () => {
    if (!recordingRef.current || !webcamRef.current || webcamRef.current.videoWidth === 0) {
      return;
    }
      ...
      canvas.toBlob(async (blob) => {
        ...
        try {
          ...
          if (prediction.label && prediction.label !== 'error') {
            // Show real-time detection (only announce if confident and different from last)
            if (prediction.confidence > 0.7) {
              const now = Date.now();
              if (
                !lastPredictionRef.current ||
                prediction.label !== lastPredictionRef.current.label ||
                now - lastPredictionRef.current.time > 800 // Show update every 0.8 seconds
              ) {
                const gestureText = GESTURE_TEXT_MAP[prediction.label] || prediction.label;
                setFeedback(`Detected: ${gestureText} (${(prediction.confidence * 100).toFixed(0)}%)`);
                setFeedbackType('warning');
                lastPredictionRef.current = { label: prediction.label, time: now };
              }
            }
  ...
  const handleStartRecording = async () => {
    ...
    setFeedback('Recording your sign... Detecting gestures...');
    setFeedbackType('warning');
    lastPredictionRef.current = null;

    // Start continuous frame capture (every 100ms for faster detection)
  ...
  const handleStopRecording = async () => {
    console.log('[Student_Practice] Stop recording called');
    console.log('[Student_Practice] Current result:', result);
    ...
    // Use the best prediction we captured during recording
    if (result) {
      console.log('[Student_Practice] Processing captured result:', result);
      try {
        await processResult(result);
      } catch (error) {
        console.error('Error processing result:', error);
        setFeedback('Error processing. Please try again.');
        setFeedbackType('error');
      }
    } else {
      // If no prediction was captured during recording, capture one final frame
      console.log('[Student_Practice] No prediction found, capturing final frame');
      
      // Try to find the video element in the DOM
      const videoElement = document.querySelector('video.webcam-feed') as HTMLVideoElement;
      ...
      if (videoElement && videoElement.videoWidth > 0 && videoElement.videoHeight > 0) {
        try {
          ...
          console.log('[Student_Practice] Canvas created:', canvas.width, 'x', canvas.height);
          ...
          if (ctx) {
            ...
            canvas.toBlob(async (blob) => {
              if (blob) {
                console.log('[Student_Practice] Blob created, sending to backend');
                const formData = new FormData();
                formData.append('file', blob, 'frame.jpg');

                try {
                  console.log('[Student_Practice] Sending to http://localhost:8000/predict/frame');
                  const response = await fetch('http://localhost:8000/predict/frame', {
                    method: 'POST',
                    body: formData
                  });

                  console.log('[Student_Practice] Response status:', response.status);
                  const prediction = await response.json();
                  console.log('[Student_Practice] Full prediction response:', JSON.stringify(prediction));
                  console.log('[Student_Practice] Prediction label:', prediction.label);
                  console.log('[Student_Practice] Prediction confidence:', prediction.confidence);

                  if (prediction && prediction.label && prediction.label !== 'error') {
                    console.log('[Student_Practice] Valid prediction received, processing...');
                    setResult(prediction);
                    await processResult(prediction);
                  } else {
                    console.error('[Student_Practice] Invalid prediction response:', prediction);
                    setFeedback(`Sign recognition failed. Response: ${JSON.stringify(prediction).substring(0, 50)}`);
                    setFeedbackType('error');
                    await speakText('Could not recognize sign. Please try again.');
                  }
                } catch (error) {
                  console.error('[Student_Practice] API Error:', error);
          ...
          console.error('[Student_Practice] Error capturing final frame:', error);
          setFeedback('Error capturing frame. Please try again.');
          setFeedbackType('error');
        }
      } else {
        console.error('[Student_Practice] Video not available', {
          exists: !!videoElement,
          videoWidth: videoElement?.videoWidth,
          videoHeight: videoElement?.videoHeight
        });
        setFeedback('Gesture not detected. Please try again.');
        setFeedbackType('error');
        await speakText('Gesture not detected. Please try again.');
      }
    }
  };
